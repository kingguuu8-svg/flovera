...
本轮真人复测确认：conversation 的流式输出、运行事件展示和长会话滚动已经进入可用状态，
但这些体验点必须作为后续回归边界固定下来，而不是当成一次性 UI polish。

已经修正并需要长期保持的体验语义：

- 如果用户已经停在 conversation 底部，final assistant response streaming 继续增长时必须持续吸附到底部。
- 如果用户主动滑离底部，系统不能强行把用户拉回最新输出。
- 已完成的静止 session 滑到底部时不能弹回顶部；这类跳动会被视为 conversation 状态恢复失败。
- 滚动目标必须是列表末尾的稳定 bottom anchor，而不是最后一条消息本身；长 assistant 消息不能被错误地按顶部对齐。
- 运行期间始终要有可见状态，至少表现为 thinking/running/tool-running/final-streaming 之一，避免用户看到长时间空白。
- 路径 chip 的语义是打开 workspace 目标文件并回到主界面预览，不是文本选择或复制入口。

这些要求属于 agent loop 产品语义：用户能否理解、等待、中断和恢复一个长任务，直接依赖这些 UI 行为是否稳定。
