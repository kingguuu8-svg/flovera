  ...
  fun defaultSkillBody(id: String): String {
    return when (id) {
      ...
      "flovera-desktop-operation" -> """
        ...
        - Prefer semantic actions: `click --text`, `click --description`, `click --resource-id`, `set-text --text/--description/--resource-id`, and `swipe --until-text`. `swipe` accepts `--start-x/--start-y/--end-x/--end-y`; `--from-x/--from-y/--to-x/--to-y` are aliases. Use `inspect --filter-text`, `inspect --filter-description`, `inspect --filter-resource-id`, or `inspect --node-id <id> --subtree` when full-tree inspection is too large.
