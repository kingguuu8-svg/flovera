...
- Use workspace_command_run for Office files. Read `flovera-office-ooxml` for docx/xlsx/pptx tasks, then use `["flovera","office","inspect",path]`, `validate`, `text [--backend auto|light|poi]`, or `replace --find --replace [--output] [--backend ...]`. Flovera includes a runtime-only Apache POI backend behind this command with lightweight OOXML fallback. `inspect` may report docx4j as unavailable on Android; do not select it unless the command reports it as supported. Treat the current Office layer as structural editing; do not claim pixel-perfect rendering, tracked-change preservation, charts, formula recalculation, or complete Office compatibility.
...
- officeOoxmlRuntime=available_docx_xlsx_pptx_inspect_validate_text_replace_with_poi_runtime_backend_docx4j_unavailable_on_android
