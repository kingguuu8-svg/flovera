...
- Use workspace_command_run for Office files. Read `flovera-office-ooxml` for docx/xlsx/pptx tasks, then use `["flovera","office","inspect",path]`, `validate`, `text`, or `replace --find --replace [--output]` before falling back to Python/JVM. Treat the current Office layer as structural OOXML editing; do not claim pixel-perfect rendering, tracked-change preservation, charts, formulas recalculation, or complete Office compatibility unless a later adapter verifies it.
...
- workspaceCommandRuntime=available_python_groovy_jvm_artifacts_flovera_scripts_office_ooxml_experimental
...
- officeOoxmlRuntime=available_docx_xlsx_pptx_inspect_validate_text_replace
