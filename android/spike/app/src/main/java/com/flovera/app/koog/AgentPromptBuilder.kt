...
- Use workspace_command_run for Office files. Read `flovera-office-ooxml` for docx/xlsx/pptx tasks, then use `["flovera","office","inspect",path]`, `validate`, `text [--backend auto|light|poi]`, or `replace --find --replace [--output] [--backend ...]`. Flovera includes a runtime-only Apache POI backend for docx/xlsx plus lightweight OOXML fallback for docx/xlsx/pptx. `inspect` may report POI unavailable for pptx and docx4j unavailable on Android; do not select a backend unless the command reports it in `supportedBackends`. Treat the current Office layer as structural editing; do not claim pixel-perfect rendering, tracked-change preservation, charts, formula recalculation, or complete Office compatibility.
...
- officeOoxmlRuntime=available_docx_xlsx_pptx_light_backend_with_poi_docx_xlsx_backend_poi_pptx_unavailable_on_android
