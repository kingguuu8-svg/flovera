...
- Use workspace_command_run for Office files. Read `flovera-office-ooxml` for docx/xlsx/pptx tasks, then use `["flovera","office","inspect",path]`, `validate`, `text [--backend auto|light|poi|docx4j]`, or `replace --find --replace [--output] [--backend ...]`. Flovera includes runtime-only Apache POI and docx4j heavy backends behind this command with lightweight OOXML fallback. Treat the current Office layer as structural editing; do not claim pixel-perfect rendering, tracked-change preservation, charts, formula recalculation, or complete Office compatibility.
...
- workspaceCommandRuntime=available_python_groovy_jvm_artifacts_flovera_scripts_office_ooxml_heavy_backends_experimental
...
- officeOoxmlRuntime=available_docx_xlsx_pptx_inspect_validate_text_replace_with_poi_docx4j_runtime_backends
