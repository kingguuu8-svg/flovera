...
object AgentPromptBuilder {
  fun systemPrompt(networkEnabled: Boolean, webSearchAvailable: Boolean): String {
    return """
      ...
      - Provider settings are profile based. Capabilities may list profile requestHooks and hook metadata such as omittedRequestFields, addedRequestFields, or defaultHeaderNames; those are app-owned transport behavior, not fields you should add to proposals. You may propose provider/model/custom OpenAI-compatible endpoint changes, customOpenAICompatibilityMode = "generic" or "ollama", and OpenRouter routing settings when provider = "openrouter", but do not invent unsupported API modes or claim a custom request body is supported.
